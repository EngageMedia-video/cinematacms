...
from files.cache_utils import get_cache_stats, health_check
...
class Command(BaseCommand):
    ...
    def clear_cache(self):
        ...
        if count > 0:
            self.stdout.write(self.style.SUCCESS(f"✓ Successfully cleared {count} cache entries"))
        else:
            self.stdout.write(self.style.WARNING("⚠ No cache entries cleared (pattern deletion may not be available)"))
    ...
    def health_check(self):
        ...
        if status == "healthy":
            self.stdout.write(self.style.SUCCESS(f"✓ Cache is healthy (latency: {latency}ms)"))
        else:
            self.stdout.write(self.style.ERROR(f"✗ Cache is unhealthy: {error} (latency: {latency}ms)"))
