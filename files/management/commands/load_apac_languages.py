    ...
    def handle(self, *args, **options):
        ...
        languages_data = [
            ...
            # Portuguese, Brazilian
            {
                ...
                "legacy_codes": ["pt-br"],
                "legacy_titles": ["Portuguese, Brazilian"],
            },
            # Tetum
            ...
            # Chinese, Cantonese (Yue) - Fix from 'zh' which was incorrectly labeled
            {
                ...
                "legacy_codes": ["zh-yue"],
                "legacy_titles": ["Chinese, Yue", "Cantonese", "Cantonese (Traditional)", "Cantonese (Simplified)"],
            },
            # Chinese, Simplified (includes generic Chinese and Mandarin as the most common form)
            {
                "code": "zh-Hans",
                "title": "Chinese (Simplified)",
                "legacy_codes": ["zh-cn", "zh-CN", "zh", "cmn", "man"],
                "legacy_titles": [
                    "Chinese, Simplified",
            ...
            # Chinese, Traditional
            {
                ...
                "legacy_codes": ["zh-tw", "zh-TW"],
                "legacy_titles": ["Chinese, Traditional", "Mandarin Chinese (Traditional)"],
            },
            # Japanese
            ...
            # Chamorro
            {
                ...
                "title": "Chamorro",
                "legacy_codes": [],
                "legacy_titles": ["Chammoro"],
            },  # Fix typo in production data
            # Dhivehi
            ...
            {"code": "automatic", "title": "Auto-generated Transcription", "legacy_codes": [], "legacy_titles": []},
            {
                ...
                "title": "Auto-generated English Translation",
                "legacy_codes": [],
                "legacy_titles": [],
            },
        ]
            ...
            if options["dry_run"]:
                ...
                if existing:
                    if existing.code != lang_data["code"] and options.get("update"):
                        self.stdout.write(
                            f"Would migrate Language code: {existing.code} -> {lang_data['code']} ({lang_data['title']})"
                        )
                ...
                if existing is None:
                    # Create new language
                    Language.objects.create(code=lang_data["code"], title=lang_data["title"])
                    created_languages += 1
                    self.stdout.write(self.style.SUCCESS(f"Created Language: {lang_data['title']}"))
                elif options["update"]:
                    ...
                    if existing.code != lang_data["code"]:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"Migrated Language code: {old_code} -> {lang_data['code']} ({lang_data['title']})"
                            )
                        )
                    ...
                    if updated:
                        ...
                        updated_languages += 1
                        if existing.code == lang_data["code"]:
                            self.stdout.write(self.style.WARNING(f"Updated Language: {lang_data['title']}"))
                else:
                    self.stdout.write(self.style.NOTICE(f"Skipped existing Language: {existing.title}"))
            ...
            if lang_data["code"] not in exclude_from_media_language:
                ...
                if options["dry_run"]:
                    ...
                    if not existing_media_langs.exists():
                        ...
                        self.stdout.write(self.style.SUCCESS(f"Created MediaLanguage: {lang_data['title']}"))
                    else:
                        ...
                        if target_exists:
                            ...
                            if duplicates_to_merge.exists() and options["update"]:
                                ...
                                for dup in duplicates_to_merge:
                                    ...
                                    self.stdout.write(
                                        self.style.WARNING(
                                            f"Merging and deleting duplicate MediaLanguage: {dup.title} (ID: {dup.id})"
                                        )
                                    )
                                    dup.delete()
                                ...
                                self.stdout.write(self.style.NOTICE(f"Skipped existing MediaLanguage: {target.title}"))
                        else:
                            ...
                            if primary.title != lang_data["title"] and options["update"]:
                                ...
                                self.stdout.write(
                                    self.style.WARNING(
                                        f"Updated MediaLanguage title: {old_title} -> {lang_data['title']}"
                                    )
                                )
                                ...
                                if duplicates:
                                    for dup in duplicates:
                                        self.stdout.write(
                                            self.style.WARNING(
                                                f"Deleting duplicate MediaLanguage: {dup.title} (ID: {dup.id})"
                                            )
                                        )
                                        dup.delete()
                            else:
                                self.stdout.write(self.style.NOTICE(f"Skipped existing MediaLanguage: {primary.title}"))
        ...
        if options["update"]:
            ...
            for old_code, new_code in consolidation_map.items():
                try:
                    ...
                    if options["dry_run"]:
                        if subtitle_count > 0:
                            self.stdout.write(f"Would migrate {subtitle_count} subtitles from {old_code} to {new_code}")
                        ...
                        if subtitle_count > 0:
                            ...
                            self.stdout.write(
                                self.style.SUCCESS(f"Migrated {updated} subtitles from {old_code} to {new_code}")
                            )
                        ...
                        self.stdout.write(self.style.SUCCESS(f"Removed duplicate language: {old_code} ({old_title})"))
