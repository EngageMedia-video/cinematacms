...
| Protect `main` | Complete | The GitHub API reports the four required checks, an up-to-date branch, one write-permission approval, latest-push approval, resolved review conversations, administrator bypass, and disabled force pushes and deletion |
...
on 2026-08-28. Administrator bypass was enabled on 2026-08-29. The rule has
these settings:
...
6. Allow repository administrators to bypass the rule.
...
The result verified on 2026-08-29 lists all four checks and reports
`up_to_date: true`, `pull_request_reviews: true`, `approvals: 1`,
`latest_push_approval: true`, `conversation_resolution: true`, `admins: false`,
