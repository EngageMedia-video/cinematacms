...
const defaults = {
	options: {
		...
		// Default VHS options for device-aware adaptive bitrate
		vhsOptions: {
			bandwidth: 1_500_000, // Default to mid-tier bandwidth
...
function filterPlayerOptions(domPlayer, opt) {
	...
	// Handle VHS options for device-aware adaptive bitrate
	if (opt.vhsOptions && opt.vhsOptions instanceof Object) {
		for (k in defaults.options.vhsOptions) {
			if (defaults.options.vhsOptions.hasOwnProperty(k)) {
				if (void 0 !== opt.vhsOptions[k]) {
					// Use provided value
					opt.vhsOptions[k] = opt.vhsOptions[k];
				} else {
					// Use default value
					opt.vhsOptions[k] = defaults.options.vhsOptions[k];
				}
			}
		}
	} else {
		opt.vhsOptions = defaults.options.vhsOptions;
	}
