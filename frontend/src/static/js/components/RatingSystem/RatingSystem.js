...
export class RatingCategory extends React.PureComponent {
	...
	onFailedRateSubmit(score) {
		this.setState({
			submitingScore: false,
			message:
				'A problem has occurred when submitting your score for the category of "<strong>' +
				this.props.title +
	...
	stars() {
		...
		while (i < 6) {
			ret.push(
				<div
					key={i}
					className={(i <= this.state.score ? 'active' : '') + (i === this.state.score ? ' selected' : '')}
				>
					<CircleIconButton onClick={this.onButtonClick.bind(this, i)}>
	...
	render() {
		return (
			<div
				className={
					'rating-category' +
					(this.state.submitingScore || null !== this.state.message ? ' submit-loading' : '')
				}
			>
				...
				<div className="rate-action">
					<button
						disabled={this.state.score === this.state.initialScore}
						onClick={this.state.score === this.state.initialScore ? null : this.submitRate}
					>
						Submit score
					</button>
				</div>
				{this.state.submitingScore ? (
				...
				{null !== this.state.message ? (
					<div className="submit-loader">
						...
						<CircleIconButton className="close-msg" onClick={this.closeRateMessage}>
...
export class RatingSystem extends React.PureComponent {
	...
	ratingCategories() {
		...
		while (i < this.props.ratings_data.length) {
			ret.push(
				<RatingCategory
					key={i}
					id={this.props.ratings_data[i].category_id}
					title={this.props.ratings_data[i].category_title}
					score={this.props.ratings_data[i].score}
				/>
			);
			i += 1;
		}
	...
	render() {
		...
		return (
			<div className={'ratings-container' + (this.state.extended ? ' extended' : '')}>
				<button className="extend-rating-form" onClick={this.onClickExtend}>
					Rate the film{' '}
					<CircleIconButton type="span">
						<i className="material-icons">
							{this.state.extended ? 'keyboard_arrow_up' : 'keyboard_arrow_down'}
						</i>
					</CircleIconButton>
				</button>
				<div className={'rating-categories'}>
					{!UserContext._currentValue.is.anonymous ? (
						...
						<div className={'rating-category need-login'}>
							<div className="category-title">
								Ιn order to be able to rate the film you need to be signed in
							</div>
							<div className="rate-action">
								<a href={logginRedirectUrl} title="Sign in">
									SIGN IN
								</a>
							</div>
						</div>
