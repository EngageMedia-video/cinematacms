...
export function MediaItemCategoriesLinks(props) {
	...
	while (i < props.categories.length) {
		catElems.push(
			<span key={'cat_elem_' + i}>
				<a
					href={formatInnerLink(props.categories[i].url, PageStore.get('config-site').url)}
					title={props.categories[i].title}
				>
					{props.categories[i].title}
				</a>
			</span>
		);
		i += 1;
...
export function MediaItemMetaViews(props) {
	return (
		<span className="item-views">{formatNumber(props.views) + ' ' + (1 >= props.views ? 'view' : 'views')}</span>
	);
}
