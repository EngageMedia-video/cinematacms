...
import PlaylistPageStore from '../../pages/PlaylistPage/store.js';
...
export function PlaylistCreationForm(props){
	...
	function onFocusDescription(){ addClassname( descriptionRef.current, 'focused' ); }
	function onBlurDescription(){ removeClassname( descriptionRef.current, 'focused' ); }
	function onChangePlaylistNameInput(){ removeClassname( nameRef.current, "invalid" ); }
	function onFocusPlaylistNameInput(){ addClassname( nameRef.current, 'focused' ); }
	function onBlurPlaylistNameInput(){ removeClassname( nameRef.current, 'focused' ); }
	...
	function onChangeDescription(){
		...
		const contentLineHeight = 0 < descriptionLineHeight ? descriptionLineHeight : parseFloat( window.getComputedStyle( descriptionInputRef.current ).lineHeight );

		descriptionInputRef.current.style.height = 3 + Math.max( 21, ( contentLineHeight * Math.floor( contentHeight / contentLineHeight ) ) ) + 'px';
	...
	return (<div className="playlist-form-wrap">
	
				<div className="playlist-form-field playlist-title" ref={ nameRef }>
					<span className="playlist-form-label">Title</span>
					<input ref={ nameInputRef } type="text" placeholder="Enter playlist title..." value={ title } onChange={onChangeTitle} onFocus={ onFocusPlaylistNameInput } onBlur={ onBlurPlaylistNameInput } onClick={ onChangePlaylistNameInput } />
				</div>
				...
				<div className="playlist-form-field playlist-description" ref={ descriptionRef }>
					...
					<textarea ref={ descriptionInputRef } rows="1" placeholder="Enter playlist description..." value={ description } onChange={onChangeDescription} onFocus={onFocusDescription} onBlur={onBlurDescription}></textarea>
				</div>
				...
				<div className="playlist-form-actions">
					<button className="cancel-btn" onClick={ onCancelPlaylistCreation }>CANCEL</button>
					<button className="create-btn" onClick={ onClickPlaylistCreate }>{ id ? "UPDATE" : "CREATE" }</button>
				</div>
