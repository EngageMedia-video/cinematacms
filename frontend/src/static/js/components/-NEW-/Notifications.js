...
function NotificationItem(props){
	...
	return ( ! isVisible ? null : <div className={ "notification-item" + ( isHidden ? " hidden" : "" ) }><div>{ props.children || null }</div></div> );
}
...
export function Notifications(){
	...
	function notificationsContent(){
		...
		const oldNotifications = visibleNotifications.map( n => {
			return <NotificationItem key={ n[0] } id={ n[0] } onHide={ onNotificationHide }>{ n[1] }</NotificationItem>;
		});
	...
	return ( ! notificationsLength ? null : <div className="notifications"><div>{ notificationsContent() }</div> </div> );
}
