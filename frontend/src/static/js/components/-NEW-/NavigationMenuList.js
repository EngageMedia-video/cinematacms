...
function NavigationMenuListItem(props){
	...
	if( -1 < iconPosIndex ){
		children[ iconPosIndex ] = <span key="Icon" className={ 'right' === props.iconPos ? 'menu-item-icon-right' : 'menu-item-icon' }>{ <MaterialIcon type={ props.icon } /> }</span>;
	}

	switch( props.itemType ){
		case 'link':
			children = <a { ...(props.linkAttr || {} ) } href={ props.link } title={ props.text || null }>{ children }</a>;
			attr.className += 'link-item' + ( props.active ? ' active' : '' );
			break;
		case 'button':
		case 'open-subpage':
			children = <button { ...(props.buttonAttr || {} ) } key="button">{ children }</button>;
			break;
		case 'label':
			children = <button { ...(props.buttonAttr || {} ) } key="button"><span>{ props.text || null }</span></button>;
			attr.className = 'label-item';
			break;
		case 'div':
			children = <div { ...(props.divAttr || {} ) } key="div">{ props.text || null }</div>;
			break;
	}
