...
function ManageItemUsername(props){
	...
	if( void 0 !== props.url ){
		if( null !== props.username && '' !== props.username ){
			return <a href={ props.url } title={ props.username }>{ props.username }</a>;
		}
	}
...
export function ManageUsersItem(props){
	...
	return (<div className="item manage-item manage-users-item">
				...
				<div className="mi-name">
					...
					<ManageItemCommentActions containerRef={actionsContainerRef} name={ props.name || props.username } onProceed={onClickProceed} />
				</div>
				<div className="mi-username"><ManageItemUsername username={ props.username } url={ props.url } /></div>
				<div className="mi-added"><ManageItemDate date={ props.add_date } /></div>
				{ props.has_roles ? <div className="mi-role">{ void 0 === props.roles ? <i className="non-available">N/A</i> : ( props.roles.length ? props.roles.join("\n") : '-' ) }</div> : null }
				{ props.has_verified ? <div className="mi-verified">{ void 0 === props.is_verified ? <i className="non-available">N/A</i> : ( props.is_verified ? <MaterialIcon type="check_circle" /> : '-' ) }</div> : null }
				{ props.has_trusted ? <div className="mi-trusted">{ void 0 === props.is_trusted ? <i className="non-available">N/A</i> : ( props.is_trusted ? <MaterialIcon type="check_circle" /> : '-' ) }</div> : null }
				<div className="mi-featured">{ void 0 === props.is_featured ? <i className="non-available">N/A</i> : ( props.is_featured ? <MaterialIcon type="check_circle" /> : '-' ) }</div>
			</div>);
}
