...
export function MediaShareEmbed(props){
	...
	const [ unitOptions, setUnitOptions ] = useState( [ { key: 'px', label: 'px' }, { key: 'percent', label: '%' } ]);
	...
	function onKeepAspectRatioChange(){
		...
		setUnitOptions(newVal ? [ { key: 'px', label: 'px' } ] : [ { key: 'px', label: 'px' }, { key: 'percent', label: '%' } ]);
	}
	...
	return ( <div className="share-embed" style={ { 'maxHeight': maxHeight + 'px' } }>
					...
					<div ref={ onRightRef } className="on-right">
						...
						<div ref={ onRightTopRef } className="on-right-top">
							<div className="on-right-top-inner">
								...
								<CircleIconButton type="button" onClick={ onClickEmbedShareExit }><MaterialIcon type="close" /></CircleIconButton>
							</div>
						</div>

						<div ref={ onRightMiddleRef } className="on-right-middle" style={ { top: rightMiddlePositionTop + 'px', bottom: rightMiddlePositionBottom + 'px' } }>
							
							<textarea readOnly value={ '<iframe width="' + ( 'percent' === embedWidthUnit ? embedWidthValue + '%' : embedWidthValue ) + '" height="' + ( 'percent' === embedHeightUnit ? embedHeightValue + '%' : embedHeightValue ) + '" src="' + links.embed + MediaPageStore.get('media-id') + '" frameborder="0" allowfullscreen></iframe>' }></textarea>
							...
							<div className="iframe-config">
								...
								<div className="iframe-config-option">
									...
									<div className="option-content">
										...
										<div className="ratio-options">
											...
											<div className="options-group" >
												<label style={ { minHeight: '36px' } }>
													<input type="checkbox" checked={ keepAspectRatio } onChange={ onKeepAspectRatioChange } />Keep aspect ratio
												</label>
											</div>
										...
										<div className="options-group" >
											<NumericInputWithUnit valueCallback={ onEmbedWidthValueChange } unitCallback={ onEmbedWidthUnitChange } label={ 'Width' } defaultValue={ parseInt( embedWidthValue, 10 ) } defaultUnit={ embedWidthUnit } minValue={ 1 } maxValue={ 99999 } units={ unitOptions } />
										</div>

										<div className="options-group" >
											<NumericInputWithUnit valueCallback={ onEmbedHeightValueChange } unitCallback={ onEmbedHeightUnitChange } label={ 'Height' } defaultValue={ parseInt( embedHeightValue, 10 ) } defaultUnit={ embedHeightUnit } minValue={ 1 } maxValue={ 99999 } units={ unitOptions } />
										</div>
									</div>
