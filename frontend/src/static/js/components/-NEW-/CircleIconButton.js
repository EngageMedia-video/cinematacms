...
export function CircleIconButton(props){
	...
	const attr = {
		...
		className: 'circle-icon-button' + ( void 0 !== props.className ? ' ' + props.className : '' ) + ( props.buttonShadow ? ' button-shadow' : '' ),
	};
	...
	if( 'link' === props.type ){
		return ( <a { ...attr } href={ props.href || null } rel={ props.rel || null }>{ children }</a> );
	}

	if( 'span' === props.type ){
		return ( <span { ...attr } onClick={ props.onClick || null }>{ children }</span> );
	}

	return ( <button { ...attr } onClick={ props.onClick || null }>{ children }</button> );
}
