...
class MediaPageStore extends EventEmitter {
	...
	dataResponse(response) {
		if (response && response.data) {
			...
			// Initialize like/dislike state from the API so that returning
	...
	likeActionResponse(response) {
		if (response) {
			if (response instanceof Error) {
				...
				// When the API returns the actual counts, update data directly
				// so the display is authoritative rather than optimistic.
				if (void 0 !== response.data.likes && MediaPageStoreData[this.id].data) {
					MediaPageStoreData[this.id].data.likes = response.data.likes;
				} else {
					// Fallback: optimistic +1 if API didn't return counts.
					MediaPageStoreData[this.id].likedInSession = true;
				}
	...
	dislikeActionResponse(response) {
		...
		if (response) {
			if (response instanceof Error) {
				...
				if (void 0 !== response.data.dislikes && MediaPageStoreData[this.id].data) {
					MediaPageStoreData[this.id].data.dislikes = response.data.dislikes;
				} else {
					MediaPageStoreData[this.id].dislikedInSession = true;
				}
	...
	get(type) {
		...
		switch (type) {
			...
			case 'media-edit-subtitle-url':
				...
				break;
			case 'media-likes':
				// Only add optimistic +1 for likes done in this page session;
				// prior likes are already included in data.likes from the API.
				tmp = MediaPageStoreData[this.id].likedInSession ? 1 : 0;
				if (tmp) {
					r =
				...
				break;
			case 'media-dislikes':
				tmp = MediaPageStoreData[this.id].dislikedInSession ? 1 : 0;
				if (tmp) {
					r =
						void 0 !== MediaPageStoreData[this.id].data &&
			...
			case 'media-production-company':
				r =
					void 0 !== MediaPageStoreData[this.id].data &&
					void 0 !== MediaPageStoreData[this.id].data.company &&
					null !== MediaPageStoreData[this.id].data.company
						? MediaPageStoreData[this.id].data.company.trim()
						: null;
				break;
			case 'media-website':
				r =
					void 0 !== MediaPageStoreData[this.id].data &&
					void 0 !== MediaPageStoreData[this.id].data.website &&
					null !== MediaPageStoreData[this.id].data.website
						? MediaPageStoreData[this.id].data.website.trim()
						: null;
				break;
			case 'display-media-license-info':
				r =
					void 0 !== MediaPageStoreData[this.id].data &&
					void 0 !== MediaPageStoreData[this.id].data.license_info;
				break;
			case 'media-license-info':
				r =
					void 0 !== MediaPageStoreData[this.id].data &&
					void 0 !== MediaPageStoreData[this.id].data.license_info &&
					Object.keys(MediaPageStoreData[this.id].data.license_info).length
						? MediaPageStoreData[this.id].data.license_info
						: null;
				break;
			case 'media-countries':
				r =
					void 0 !== MediaPageStoreData[this.id].data &&
					void 0 !== MediaPageStoreData[this.id].data.media_country_info
						? MediaPageStoreData[this.id].data.media_country_info
						: [];
				break;
			case 'media-languages':
				r =
					void 0 !== MediaPageStoreData[this.id].data &&
					void 0 !== MediaPageStoreData[this.id].data.media_language_info
						? MediaPageStoreData[this.id].data.media_language_info
						: [];
				break;
			...
			case 'media-topics':
				r =
					void 0 !== MediaPageStoreData[this.id].data &&
					void 0 !== MediaPageStoreData[this.id].data.topics_info
