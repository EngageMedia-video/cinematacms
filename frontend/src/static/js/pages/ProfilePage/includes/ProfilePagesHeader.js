...
class ProfileSearchBar extends React.PureComponent {
	constructor(props) {
		...
		/*if( this.state.queryVal && '' !== this.state.queryVal ){
			this.state.visibleForm = true;
		}*/
		...
		this.setState(
			...
			function () {
				if (this.updateTimeout) {
					// console.log( 'drop', this.pendingEvent.target.value || '', this.updateTimeout );
					return;
				}

				// console.log( 'pass', this.pendingEvent.target.value || '' );
	...
	render() {
		...
		/*if( 'author-home' === PageStore.get('current-page') ){
			return <div>
						<span><CircleIconButton buttonShadow={false} onClick={ this.hideForm }><i className="material-icons">search</i></CircleIconButton></span>
						<span><input ref='SearchInput' type="text" name="aq" placeholder="Search" aria-label="Search" value={ this.state.queryVal } onChange={ this.onChange } onFocus={ this.onInputFocus } onBlur={ this.onInputBlur } /></span>
					</div>;
		}*/
...
class NavMenuInlineTabs extends React.PureComponent {
	...
	render() {
		// console.log( PageStore.get('config-options').pages );
		...
		// return <nav ref="tabsNav" className="profile-nav items-list-outer list-inline list-slider">
		// 			<div className="profile-nav-inner items-list-outer">
		// 				{ this.state.displayPrev ? this.previousBtn : null }
		// 				<ul className="items-list-wrap" ref="itemsListWrap">
		// 					{ /*UserContext._currentValue.pages.home ?*/ <li className={ 'home' === this.props.type ? 'active' : null }><a href={ LinksContext._currentValue.profile.home } title="Media">{ this.userIsAuthor ? 'MY ' : '' }MEDIA</a></li> /*: null*/ }
		// 					{/*UserContext._currentValue.pages.media ? <li className={ 'videos' === this.props.type ? 'active' : null }><a href={ LinksContext._currentValue.profile.media } title="Media">MEDIA</a></li> : null*/}
		// 					{ UserContext._currentValue.can.saveMedia ? <li className={ 'playlists' === this.props.type ? 'active' : null }><a href={ LinksContext._currentValue.profile.playlists } title="Playlists">{ this.userIsAuthor ? 'MY ' : '' }PLAYLISTS</a></li> : null }
		// 					{ /*UserContext._currentValue.pages.about ?*/ <li className={ 'about' === this.props.type ? 'active' : null }><a href={ LinksContext._currentValue.profile.about } title="About">ABOUT</a></li> /*: null*/ }
		// 					{ PageStore.get('config-options').pages.profile.includeHistory && this.userIsAuthor ? <li className={ 'history' === this.props.type ? 'active' : null }><a href={ LinksContext._currentValue.user.history } title="History">HISTORY</a></li> : null }
		// 					{ PageStore.get('config-options').pages.profile.includeLikedMedia && this.userIsAuthor ? <li className={ 'liked' === this.props.type ? 'active' : null }><a href={ LinksContext._currentValue.user.liked } title="Liked media">LIKED MEDIA</a></li> : null }
		// 				</ul>
		// 				{ this.state.displayNext ? this.nextBtn : null }
		// 			</div>
		// 		</nav>;
