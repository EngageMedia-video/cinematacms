	...
	pageContent() {
		...
		if (this.state.author) {
			...
			if (void 0 !== this.state.author.location_info && this.state.author.location_info.length) {
				...
				while (i < this.state.author.location_info.length) {
					if (
						...
						locations.push(
							<a
								key={i}
								href={formatInnerLink(
									this.state.author.location_info[i].url,
				...
				// TODO: Doesn't really need. Remains only for backward compatibility.
				details.push(
					<li key={'location'}>
						<span>Location:</span>
			...
			this.state.author ? (
				<ProfilePagesContent key="ProfilePagesContent" enabledContactForm={this.enabledContactForm}>
					<div className="media-list-wrapper items-list-ver  profile-about-content ">
						{null === description && 0 < details.length ? null : PageStore.get('config-options').pages
								.profile.htmlInDescription ? (
