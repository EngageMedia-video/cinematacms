...
import { formatNumber, quickSort, greaterCommonDivision } from '../functions';
import { addClassname, removeClassname } from '../functions/dom';

let _MediaDurationInfo = null;

function getMediaDurationInfo() {
	if (!_MediaDurationInfo) {
		_MediaDurationInfo = new MediaDurationInfo();
	}
	return _MediaDurationInfo;
}
	...
	while (i < itemsData.length) {
		...
		items[i].innerHTML =
			...
			itemContent(
				...
				formatNumber(itemsData[i].views) + ' ' + (1 >= itemsData[i].views ? 'view' : 'views'),
				hideViews
			) +
			'</a>';
...
export default function PlayerRecommendedMedia(itemsData, inEmbed, hideViews) {
	...
	function disableItemsRevealAnim() {
		setTimeout(
			...
			domElems.items.length * 75 + 200
		); // @note: 200ms is reveal animation duration and 75ms is reveal animation delay (in CSS, with class selector '.before-more-media-item-load' ).
	}
