export function SearchResultSection({ title, items, hasMore, query, renderItem, onSelect, withDivider = true }) {
	...
	return (
		<section
			className={
				'px-3 py-3 ' +
				(withDivider ? 'border-t border-cinemata-pacific-deep-700/60 first:border-t-0' : '')
			}
			aria-label={title}
		>
			...
			<div role="listbox" aria-label={title} className="flex flex-col gap-1">
				{items.map((item, idx) => renderItem(item, idx, onSelect))}
			</div>
			{hasMore ? (
				<a
					href={showMoreHref}
