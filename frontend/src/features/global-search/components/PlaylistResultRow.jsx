...
export function PlaylistResultRow({ item, onSelect }) {
	...
	return (
		...
		>
			<span className="relative flex h-[44px] w-[78px] shrink-0 items-center justify-center overflow-hidden rounded-[6px] bg-bg-chrome-hover">
				{thumb ? (
					<img src={thumb} alt="" loading="lazy" className="h-full w-full object-cover" />
				) : (
					<Text aria-hidden="true" variant="body-12" as="span" className="m-0 text-text-on-chrome-muted">
						List
					</Text>
				)}
				<span
