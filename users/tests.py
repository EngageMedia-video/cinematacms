...
class LoginTestCase(TestCase):
    ...
    def test_superuser_redirect(self):
        ...
        response = self.client.post(
            login_url,
            {"login": self.superuser_username, "password": self.superuser_password},
        )

        # Check if the login was successful (should redirect to home page)
        self.assertRedirects(
            response, "/accounts/2fa/totp/activate", fetch_redirect_response=False
        )
...
class MFATestCase(TestCase):
    ...
    def test_superuser_mfa_redirect_on_first_login(self):
        ...
        response = self.client.post(
            login_url,
            {"login": self.superuser.username, "password": "superpassword123"},
        )
    ...
    def test_regular_user_no_mfa_redirect(self):
        ...
        response = self.client.post(
            login_url,
            {"login": self.regular_user.username, "password": "testpassword123"},
        )
    ...
    def test_editor_mfa_redirect(self):
        ...
        login_url = reverse("account_login")
        response = self.client.post(
            login_url, {"login": self.editor_user.username, "password": "editorpass123"}
        )
    ...
    def test_manager_mfa_redirect(self):
        ...
        response = self.client.post(
            login_url,
            {"login": self.manager_user.username, "password": "managerpass123"},
        )
...
class TestIndexPageFeaturedValidation(TestCase):
    ...
    def test_external_links_rejected(self):
        ...
        content = '<a href="https://evil.com">Bad Link</a>'
