        ...
        try:
            ...
            if options.get("name"):
                ...
                if portal_name is None:
                    self.stdout.write(
                        self.style.WARNING('PORTAL_NAME not found in settings. Using default "CinemataCMS"')
                    )
            ...
            if domain:
                site, created = Site.objects.get_or_create(pk=site_id, defaults={"name": portal_name, "domain": domain})
            else:
                # If no domain provided, just get or create with name
                site, created = Site.objects.get_or_create(
                    pk=site_id, defaults={"name": portal_name, "domain": "example.com"}
                )
